...
#define CONTAINED_IN(x) Is_codepoint_in_ranges(cp, (x), COUNT_OF((x)))
...
bool May_non_ascii_codepoint_start_id(lang_ver_t lang_ver, uint32_t cp)
{
	...
	else if (lang_ver == lang_ver_c2x)
	{
		if (CONTAINED_IN(xid_start))
			return true;

		if (CONTAINED_IN(math_start))
			return true;

		return false;
	}
	else if (!Does_non_ascii_codepoint_extend_id(lang_ver, cp))
	{
		return false;
	}
	else if (lang_ver == lang_ver_c11)
	{
		return !CONTAINED_IN(c11_disallowed_initial);
	}
	else
	{
		return !CONTAINED_IN(c99_disallowed_initial);
	}
}
...
bool Does_non_ascii_codepoint_extend_id(lang_ver_t lang_ver, uint32_t cp)
{
	...
	else if (lang_ver == lang_ver_c2x)
	{
		...
		if (CONTAINED_IN(xid_continue))
			return true;

		if (CONTAINED_IN(math_start))
			return true;

		if (CONTAINED_IN(math_continue))
			return true;
	...
	{
		return CONTAINED_IN(c11_allowed);
	}
	else
	{
		return CONTAINED_IN(c99_allowed);
	}
}

bool Is_unicode_whitespace(uint32_t cp)
{
	return CONTAINED_IN(unicode_whitespace);
}